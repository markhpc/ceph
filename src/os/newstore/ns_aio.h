...
struct ns_aio_t {
  ...
  ns_aio_t(void *p, int f) : priv(p), fd(f) {
...
struct ns_aio_queue_t {
  int max_iodepth;
  io_context_t ctx;

  ns_aio_queue_t(unsigned max_iodepth)
  ...
  int submit(ns_aio_t &aio, int *retries) {
    int attempts = 10;
    iocb *piocb = &aio.iocb;
    while (true) {
  ...
  int get_next_completed(int timeout_ms, ns_aio_t **paio, int max) {
    io_event event[max];
    struct timespec t = {
      timeout_ms / 1000,
