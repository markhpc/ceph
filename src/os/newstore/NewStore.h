...
public:
  ...
  struct OnodeHashLRU {
    ...
    OnodeHashLRU(CephContext* cct) : cct(cct), lock("NewStore::OnodeHashLRU::lock") {}
  ...
  struct TransContext {
    ...
    list<aio_t> pending_aios;    ///< not yet submitted
    list<aio_t> submitted_aios;  ///< submitting or submitted
    bufferlist aio_bl;  // just a pile of refs
    std::atomic<uint32_t> num_aio;
