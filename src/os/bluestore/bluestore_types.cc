...
*/
  ...
  ::encode(flags, ap);
  if (is_compressed()) {
    ::encode(compressed_length_orig, ap);
    ::encode(compressed_length, ap);
  }
  if (has_csum()) {
    ::encode(csum_type, ap);
...
void bluestore_blob_t::decode(bufferlist::iterator& p)
{
  ...
  ::decode(flags, p);
  if (is_compressed()) {
    ::decode(compressed_length_orig, p);
...
void small_encode(const map<uint64_t,bluestore_lextent_t>& extents, bufferlist& bl)
{
  ...
  if (n) {
    ...
    for (; i < n-n%encodes; i+=encodes) {
      ...
      for (uint32_t j = 0; j < encodes; ++j) {
        ++p;
        ::encode((uint64_t)p->first - pos, ap);
        p->second.encode(ap);
        pos = p->first;      
      }
    }
