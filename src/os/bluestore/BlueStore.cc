    ...
    WriteContext *wctx)
{
  ...
  while (ep != o->extent_map.extent_map.end()) {
    ...
    if (!head_read && !tail_read &&
      ...
      // if blob wasn't extended - need to check for pending writes desiring to
      ...
      // do_alloc_write().
      bool conflicting = llen == b->get_blob().get_logical_length() &&
                           wctx->has_conflict(b, offset0, offset0 + alloc_len, min_alloc_size);
      if (!conflicting) {
