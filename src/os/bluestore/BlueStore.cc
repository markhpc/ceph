  ...
  uint32_t op_flags)
{
  ...
  if (might_need_reformatting) {
    string val;
    c->pool_opts.get(pool_opts_t::DATA_LAYOUT_REFORMATTING, &val);
    dout(10) << __func__ << " layout reformatting = " << val
            << " span stat {" << span_stat << "}"
            << dendl;
    _dump_onode<30>(cct, *o);
	...
	if (wctx.compress &&
      ...
      if (!might_need_recompress && (val == "defragment" || val == "both")) {
