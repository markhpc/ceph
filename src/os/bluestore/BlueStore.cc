  ...
  float bytes_per_onode)
{
  ...
  if (current <= target_bytes) {
    dout(10) << __func__
	     ...
	     << " ratio " << target_meta_ratio << " is "
	     << pretty_si_t(target_meta) << " meta + "
	     << pretty_si_t(target_buffer) << " buffer,"
	     << " current " << pretty_si_t(current) << " is "
	     << pretty_si_t(current_meta) << " meta + "
	     << pretty_si_t(current_buffer) << " buffer"
	     << dendl;
    return;
  }
  ...
  dout(10) << __func__
	   ...
	   << pretty_si_t(current_meta) << " meta + "
	   << pretty_si_t(current_buffer) << " buffer,"
	   << " need_to_free " << pretty_si_t(need_to_free) << " in "
	   << pretty_si_t(free_meta) << " meta + "
	   << pretty_si_t(free_buffer) << " buffer"
...
BlueStore::Collection::Collection(BlueStore *ns, Cache *c, coll_t cid)
  : store(ns),
    ...
    shared_blob_set(MAX(16,
			g_conf->bluestore_cache_size /
			store->cache_shards.size() *
			g_conf->bluestore_cache_meta_ratio /
			sizeof(Onode) * 10 *   /* this is a wild guess :( */
