...
int ForestDBStore::set_merge_operator(
  const string& prefix,
...
int ForestDBStore::init(string option_str)
{
  config.buffercache_size = g_conf->forrestdb_buffercache_size;
/*
...
int ForestDBStore::create_and_open(ostream &out) {
...
int ForestDBStore::do_open(ostream &out, bool create_if_missing)
{
  fdb_status status;

  status = fdb_open(&fhandle, path + "/forestdb.db", &config);
...
int ForestDBStore::_test_init(const string& dir)
{
  fdb_config config;
...
//  memset(&config, 0, sizeof(fdb_config)));
  ...
  status = fdb_open(&fhandle, path + "/forest.db", &config);
  assert(status == FDB_RESULT_SUCCESS);
  
  status = fdb_kvs_open(fhandle, &kvhandle, &kvs_config);
  assert(status == FDB_RESULT_SUCCESS);

  fdb_doc *doc;
  status = fdb_doc_create(&doc, k, k.length(), NULL, 0, v, v.length());
  assert(status == FDB_RESULT_SUCCESS);
...
ForestDBStore::ForestDBStore(CephContext *c, const string &path) :
  cct(c),
  logger(NULL),
  path(path),
{
