...
// These macros all handle byte-ordering automatically for you :) All
// external serializations are in little endian order
...
#define DEFINE_DENC_SIMPLE(itype, etype)				\
  template<>								\
  struct denc_traits<itype> {						\
    ...
    static void encode(const itype &o, buffer::list::contiguous_appender& p, \
		       uint64_t f=0) {					\
      *(etype *)p.get_ptr_add(sizeof(etype)) = static_cast<etype>(o);	\
    }									\
    static void decode(itype& o, buffer::ptr::iterator &p) {		\
      o = *(etype*)p.c_str_add(sizeof(etype));				\
    }									\
  };

DEFINE_DENC_SIMPLE(uint8_t, uint8_t);
DEFINE_DENC_SIMPLE(int8_t, int8_t);
DEFINE_DENC_SIMPLE(uint16_t, __le16);
DEFINE_DENC_SIMPLE(int16_t, __le16);
DEFINE_DENC_SIMPLE(uint32_t, __le32);
DEFINE_DENC_SIMPLE(int32_t, __le32);
DEFINE_DENC_SIMPLE(uint64_t, __le64);
DEFINE_DENC_SIMPLE(int64_t, __le64);
...
inline void denc_varint(T& v, bufferptr::iterator& p) {
  uint8_t byte = *(__u8*)p.c_str_add(1);
  v = byte & 0x7f;
  int shift = 7;
  while (byte & 0x80) {
    byte = *(__u8*)p.c_str_add(1);
    v |= (T)(byte & 0x7f) << shift;
    shift += 7;
  }
}
...
inline void denc_lba(uint64_t& v, bufferptr::iterator& p) {
  uint32_t word = *(__le32*)p.c_str_add(sizeof(uint32_t));
  int shift;
  switch (word & 7) {
  case 0:
  case 2:
  ...
  bufferlist::iterator& p)
{
  ...
  t.copy(p.get_bl().length() - p.get_off(), tmp);
  auto cp = tmp.begin();
  traits::decode(o, cp);
...
#define DENC_HELPERS							\
  static void _denc_start(__u8 v, __u8 compat, size_t& p,		\
			  __u8 *struct_v,				\
			  __u8 *struct_compat,				\
			  char **struct_pos,				\
			  uint32_t *struct_len) {			\
  ...
  static void _denc_start(__u8 v, __u8 compat,				\
    ...
    denc(*struct_compat, p);						\
    denc(*struct_len, p);						\
    *struct_pos = const_cast<char*>(p.c_str());				\
  }									\
  static void _denc_finish(buffer::ptr::iterator& p,			\
			   __u8 *struct_v, __u8 *struct_compat,		\
			   char **struct_pos,				\
			   uint32_t *struct_len) {			\
    const char *pos = p.c_str();					\
    char *end = *struct_pos + *struct_len;				\
    assert(pos <= end);							\
    if (pos < end) {							\
      p.advance(end - pos);						\
    }									\
...
#define DENC_START(v, compat, p)					\
  ...
  char *struct_pos;							\
  uint32_t struct_len;							\
  _denc_start(v, compat, p,						\
	      &struct_v, &struct_compat, &struct_pos, &struct_len);	\
  do {

#define DENC_FINISH(p)							\
  } while (false);							\
  _denc_finish(p, &struct_v, &struct_compat, &struct_pos, &struct_len);
