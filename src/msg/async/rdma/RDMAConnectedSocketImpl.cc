...
ssize_t RDMAConnectedSocketImpl::submit(bool more)
{
  ...
  auto fill_tx_via_copy = [this](std::vector<Chunk*> &tx_buffers, unsigned bytes,
                                 buffer::ptrlist::const_iterator &start,
                                 buffer::ptrlist::const_iterator &end) -> unsigned {
    assert(start != end);
  ...
  buffer::ptrlist::const_iterator it = pending_bl.buffers().begin();
  buffer::ptrlist::const_iterator copy_it = it;
  unsigned total = 0;
