...
namespace PriorityCache {
  ...
  void Manager::insert(std::string name, std::shared_ptr<PriCache> c) {
    ceph_assert(!caches.count(name));
    ceph_assert(!indexes.count(name));

    caches.emplace(name, c);

    // TODO: If we every assign more than
  ...
  void Manager::balance()
  {
    ...
    for (auto it = caches.begin(); it != caches.end(); it++) {
      ...
      // And rotate the bins for the next interval
